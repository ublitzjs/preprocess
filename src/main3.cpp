...
//  }
  ...
  Napi::Array init(const Napi::CallbackInfo &info){
    ...
    maxChunkSize = info[1].As<Napi::Number>().Uint32Value();
    cachingEmitter = Napi::Persistent(info[2].As<Napi::Function>());
    return instructions;
  }
  ...
  //  )=>void
  //): boolean, // isGlobal
  void createCache(const Napi::CallbackInfo& info){
    ...
    int64_t fileSize;
    if(
      ((input = cross_os::OpenFileRead(templateName.data())) == cross_os::invalid_descriptor)
      ||(fileSize = cross_os::GetFileSize(input)) == cross_os::invalid_file_size   
    ){
      cb.Call({Napi::Number::New(env, Status::CantUseFile)});
      return;
    }
    cache* cacheStruct = new cache(templateName, info[1].As<Napi::Boolean>().Value(), info[1].As<Napi::Boolean>().Value());

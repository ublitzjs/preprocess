...
async function render(
  ...
  if(!err) shared.currentId = null
  var cache
  for(var i = 0; i<shared.caches.length; i++){
    cache = shared.caches[i];
    if(cache && cache.status != StatusEnum.streamed){
      if(shared.currentId == i) {
        cache.status = StatusEnum.bad;
      }
      if(!cache.busy--) delete allCaches[params.templates[i]]
    }
  }
  if(err) { 
    return new Error("template " + params.templates[shared.currentId] + " failed: " + err.message); 
  }
  if(shared.result) await params.progressCB(shared.result)
}

export { compile, render, manualCreateCache, tryToClearCache, allSyntax}
